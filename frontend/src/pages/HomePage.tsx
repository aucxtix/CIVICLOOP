  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-background text-foreground font-sans overflow-x-hidden selection:bg-green-200">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 lg:px-12 flex items-center justify-between bg-card/80 backdrop-blur-xl border-b border-border/50">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <span className="text-xl font-extrabold tracking-tight leading-none">CivicLoop</span>
        ...
        <div className="hidden lg:flex items-center gap-8 text-sm font-bold text-muted-foreground">
          ...
          <a href="#how-it-works" className="hover:text-green-600 transition-colors pb-1">How It Works</a>
          <a href="#features" className="hover:text-green-600 transition-colors pb-1">Features</a>
          <a href="#impact" className="hover:text-green-600 transition-colors pb-1">Community Impact</a>
        </div>
        <div className="flex items-center gap-4">
          <ModeToggle />
          <Button asChild variant="outline" className="hidden sm:inline-flex border-border text-foreground hover:bg-background font-bold rounded-full px-6 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
            <Link to="/login">Log In</Link>
          </Button>
          <Button asChild className="bg-green-600 text-white hover:bg-green-700 font-bold rounded-full px-6 shadow-lg shadow-green-600/20">
      ...
      <div className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 px-6 lg:px-12 max-w-[1600px] mx-auto flex flex-col lg:flex-row items-center gap-12">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-50 border border-green-100 text-green-700 text-xs font-bold uppercase tracking-wider">
          ...
          <h1 className="text-6xl lg:text-7xl xl:text-8xl font-black tracking-tighter leading-[1.05] text-foreground">
            Smarter Waste.<br />
            <span className="text-green-600">Cleaner Communities.</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
            <Button asChild size="lg" className="w-full sm:w-auto h-14 px-8 text-lg bg-green-600 text-white hover:bg-green-700 font-bold rounded-full shadow-xl shadow-green-600/20">
              <Link to="/citizen/classify"><Camera className="w-5 h-5 mr-2" /> Classify Waste</Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="w-full sm:w-auto h-14 px-8 text-lg border-border text-foreground hover:bg-background font-bold rounded-full bg-card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-12 border-t border-border/60 mt-12">
            {[
              { icon: Zap, title: "AI Classification", desc: "Identify waste correctly with AI accuracy" },
              { icon: MapPin, title: "Smart Reporting", desc: "Report garbage or illegal dumping in seconds" },
              { icon: Truck, title: "Collection Mgmt", desc: "Request pickup and track collection status" },
              { icon: TrendingUp, title: "Track Impact", desc: "Monitor contribution and earn green points" }
            ].map((feature, i) => (
              <motion.div 
                key={i}
              ...
              >
                <div className="w-12 h-12 rounded-full bg-green-50 flex items-center justify-center text-green-600 border border-green-100 mb-3">
          ...
          <div className="relative rounded-3xl bg-card border border-border/60 shadow-2xl shadow-slate-200/50 overflow-hidden">
            ...
            <div className="flex h-[600px] bg-background">
              ...
              <div className="w-48 border-r border-border bg-card p-4 space-y-2">
                <div className="h-8 rounded bg-green-50 w-full flex items-center px-3 border-l-4 border-green-600"><span className="w-3/4 h-2 bg-green-600 rounded"></span></div>
                <div className="h-8 rounded w-full flex items-center px-3"><span className="w-2/3 h-2 bg-slate-200 rounded"></span></div>
                <div className="h-8 rounded w-full flex items-center px-3"><span className="w-1/2 h-2 bg-slate-200 rounded"></span></div>
                <div className="h-8 rounded w-full flex items-center px-3"><span className="w-3/4 h-2 bg-slate-200 rounded"></span></div>
                <div className="h-8 rounded w-full flex items-center px-3"><span className="w-2/3 h-2 bg-slate-200 rounded"></span></div>
              </div>
      ...
      <div className="bg-card py-24 relative z-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div id="how-it-works">
            <h2 className="text-3xl font-bold mb-2">How <span className="text-green-600">CivicLoop</span> Works</h2>
            ...
            <div className="space-y-8 relative before:absolute before:inset-0 before:ml-6 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
              {[
                { step: "1. Capture", icon: Camera, desc: "Capture or upload a photo of the waste." },
                { step: "2. Classify", icon: Zap, desc: "AI identifies the waste type and best disposal method." },
                { step: "3. Act", icon: Truck, desc: "Report, request pickup, or find a nearby collection point." },
                { step: "4. Track Impact", icon: Leaf, desc: "Track your actions and earn green points." }
              ].map((item, i) => (
                <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-white bg-green-100 text-green-600 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
                    <item.icon className="w-5 h-5" />
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] p-6 rounded-2xl bg-background border border-border shadow-sm transition-all hover:shadow-md">
          ...
          <div>
            <h2 className="text-3xl font-bold mb-2">Why Choose <span className="text-green-600">CivicLoop</span>?</h2>
            <p className="text-muted-foreground mb-10">Built for scale, designed for impact.</p>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {[
                { title: "Easy to Use", desc: "Simple, intuitive and designed for everyone.", icon: Smartphone },
                { title: "AI-Powered", desc: "High accuracy waste classification.", icon: Zap },
                { title: "Community Driven", desc: "Stronger together for cleaner neighborhoods.", icon: Globe },
                { title: "Verified & Transparent", desc: "Real-time tracking and verified actions.", icon: ShieldCheck },
                { title: "Rewards & Impact", desc: "Earn green points and redeem exciting rewards.", icon: TrendingUp },
                { title: "Sustainable Future", desc: "Small actions today lead to a big impact tomorrow.", icon: Recycle }
              ].map((feature, i) => (
                <div key={i} className="bg-background rounded-2xl p-6 border border-border flex items-start gap-4 hover:border-green-200 transition-colors">
                  <div className="p-3 bg-card rounded-xl text-green-600 shadow-sm border border-border shrink-0">
                    <feature.icon className="w-5 h-5" />
                  </div>
                  <div>
      ...
      {/* Footer Banner */}
      <div className="bg-green-950 text-white py-12">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex flex-col lg:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-green-200 text-sm mt-1">Join thousands of citizens making a real difference in their communities.</p>
            </div>
          </div>
          <div className="flex gap-4 w-full lg:w-auto">
            <Button asChild size="lg" className="w-full lg:w-auto bg-green-500 hover:bg-green-400 text-white font-bold rounded-full">
              <Link to="/register">Get Started</Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="w-full lg:w-auto border-green-700 text-green-400 hover:bg-green-900 hover:text-white font-bold rounded-full bg-transparent">
              <a href="#how-it-works">Learn More</a>
            </Button>
          </div>
          <div className="flex gap-8 border-t lg:border-t-0 lg:border-l border-green-800 pt-8 lg:pt-0 lg:pl-8 w-full lg:w-auto justify-between">
             <div className="text-center">
               <div className="text-2xl font-black">10K+</div>
               <div className="text-xs text-green-400 uppercase tracking-wider font-bold mt-1">Active Citizens</div>
             </div>
             <div className="text-center">
               <div className="text-2xl font-black">25K+</div>
               <div className="text-xs text-green-400 uppercase tracking-wider font-bold mt-1">Reports</div>
             </div>
             <div className="text-center">
               <div className="text-2xl font-black">1.5M+</div>
               <div className="text-xs text-green-400 uppercase tracking-wider font-bold mt-1">kg Managed</div>
             </div>
          </div>
        </div>
      </div>
